...
fn main() -> Result<(),Box<dyn Error>> {
    ...
    let mut tm_events = vec![TMEvent{id: "284753".to_string(), offers: 0}, TMEvent{id: "280409".to_string(), offers: 0}];
        ...
        while event::poll(Duration::default())? {
            if let Event::Key(key_event) = event::read()? {
                ...
                match key_event.code {
                    KeyCode::Esc | KeyCode::Char('q') => { // User pressed ESC or 'q', breaking the main loop
                        break 'mainloop
                    }

                    KeyCode::Char('p') => { // User pressed 'p', forcing an update of the events

                        println!("User forced polling of all events");

                        match update_events(&mut tm_events) { // running update function & print a message for feedback. passing the main variable as mutable borrow, so the function can actually change the variable
...
fn update_events(evs: &mut Vec<TMEvent>) -> Result<(),Box<dyn Error>> {
    for ev in evs.iter_mut() { // Iterating over all the events
        match poll_event(&ev.id) { // checking if was succes
            Ok(n) => {
...
fn poll_event(id: &String) -> Result<usize, Box<dyn Error>> {
   ...
   let resp: TMResponse = reqwest::blocking::get(request_url)?
       .json()?; // Doing the update, note the ? as this will propogate errors up!
